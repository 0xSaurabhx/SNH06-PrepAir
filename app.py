...
def scrapeposts():
    ...
    #with open('demo.json') as f:
        ...
        num_appreciations = [item['num_appreciations'] for item in data]
        num_reposts = [item['num_reposts'] for item in data]
        num_empathy = [item['num_empathy'] for item in data]
        num_reposts = [item['num_reposts'] for item in data]
        st.divider()
        ...
        with p1:
            ...
            st.divider()
        with p2:
            st.write("Total Appreciations")
            st.title(sum(num_appreciations))
            st.divider()
        with p3:
            st.write("Total Impressions")
            total_impressions = sum(num_likes)+sum(num_appreciations)+sum(num_empathy)+sum(num_reposts)
            st.title(total_impressions)
            st.divider()
        with p4:
            st.write("Total Engagements")
            total_engagements = sum(num_likes)+sum(num_appreciations)+sum(num_empathy)
        ...
        st.area_chart(chart_data, x="Number of Appreciations", y=["Number of Likes","Number of Reposts"])
    #else:
    #    st.write("ERROR 404")
